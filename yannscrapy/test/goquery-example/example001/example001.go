...
func main() {
	...
	body, err := ioutil.ReadFile("E:/liyuan-github/yannscrapy/aa.html")
	if err != nil {
		fmt.Errorf("%v\n", err)
		return
	}
	...
	v := dom.Find("div[class='product-cv'] a span[class='btn-cv__txt']").Eq(0).Text()
	v = strings.Trim(v, " ")
	fmt.Printf("v=%s\n", v)
	...
	v = dom.Find("div[class='product-cv'] button span[class='btn-cv__txt']").Eq(0).Text()
	v = strings.Trim(v, " ")
